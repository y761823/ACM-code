...
Line original[MAXN], newLine[MAXN], deq[MAXN];

bool check(double r) {
    for(int i = 0; i < n; ++i) newLine[i] = original[i];
    for(int i = 0; i < n; ++i) moveRight(newLine[i], r);
...
int main() {
    while(scanf("%d", &n) != EOF && n) {
        ...
        p[n] = p[0];
        for(int i = 0; i < n; ++i) original[i] = Line(p[i + 1], p[i]);
        for(int i = 0; i < n; ++i) original[i].makeAg();
        double l = 0, r = 1e9;
        for(int i = 0; i < 100; ++i) {
            double mid = (l + r) / 2;
            if(check(mid)) l = mid;
